...
contract SeriesFactoryTest is Base {
    function test_create_usdAndRwa() public {
        address usd = factory.create_series(USD, 1250e18, block.timestamp + 28 days);
        address xau = factory.create_series(xauId, 0.5e18, block.timestamp + 28 days);
        ...
        assertEq(IOptionSeries(usd).ASSET(), USD);
        assertEq(IOptionSeries(xau).ASSET(), xauId);
    }

    function test_create_dedupes() public {
        uint256 m = block.timestamp + 28 days;
        address a = factory.create_series(USD, 1250e18, m);
        address b = factory.create_series(USD, 1250e18, m);
        assertEq(a, b);
    ...
    function test_create_distinctParamsDistinctSeries() public {
        uint256 m = block.timestamp + 28 days;
        address a = factory.create_series(USD, 1250e18, m);
        address b = factory.create_series(USD, 1251e18, m);
        address c = factory.create_series(USD, 1250e18, m + 1);
        assertTrue(a != b && b != c && a != c);
    }

    function test_create_revertsUnregistered() public {
        vm.expectRevert();
        factory.create_series(keccak256("nope"), 1e18, block.timestamp + 28 days);
    }

    function test_create_revertsBadTerm() public {
        vm.expectRevert();
        factory.create_series(USD, 1e18, block.timestamp + 30 minutes);
        vm.expectRevert();
        factory.create_series(USD, 1e18, block.timestamp + 6 * 365 days);
    }

    function test_create_revertsZeroStrike() public {
        vm.expectRevert();
        factory.create_series(USD, 0, block.timestamp + 28 days);
    }

    function test_seriesTokensWiredCorrectly() public {
        IOptionSeries s = IOptionSeries(
            factory.create_series(xauId, 0.5e18, block.timestamp + 28 days)
        );
